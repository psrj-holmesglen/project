...
$vDescriptions = array(
        ...
        3 => "Please enter a valid URL address in the format http://www.domain.com(.net, .org, etc.)", // url
        ...
        11 => "Field can only contain alphabetic characters.", //string inputs
		...
		16 => "Field can only contain alphabetic,numeric,_ or . characters." //user name inputs
);
...
function vIsUserName($input)
{
    global $vErrCode, $vDescVariables;
    // Check for blank string first.
    if (vIsBlank($input))
        return false;
    // Check to see if all Field can only contain alphabetic,numeric,_ or . characters only.
