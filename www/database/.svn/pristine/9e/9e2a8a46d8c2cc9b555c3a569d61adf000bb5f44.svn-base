 ...
 * Last updated: 17-05-14 by Jennifer de Peyrecave.
	...
	if (empty($_FILES["file"]["name"])){
		...
		if ((($_FILES["file"]["type"] == "image/gif")
			...
			&& in_array($extension, $allowedExts)) {
				//echo "check image type";
					//check if file already exists in Images folder - this will not be needed when name is set as ID
					if (file_exists( $uploLoc . $_FILES["file"]["name"])){
						//echo "Already exists";
						$vErrCode = 13;
						return true;
						//return false;
					}else{//save the uploaded file into a permanent folder
						//echo "upload to folder";
